...
def _r7q_runtime_cli_contract_patch():
    """
    R7Q patch purpose:
    - Preserve existing launcher behavior.
    - Avoid R7 exit-code-2 argparse failure when manual runtime roots are supplied by environment.
    ...
    def _r7q_dest_key(dest):
        return str(dest or "").lower().replace("-", "_")

    def _r7q_value_for_dest(dest):
        key = _r7q_dest_key(dest)

        if "output" in key and "root" in key:
            return _r7q_env_first([
                "MODULEC_OUTPUT_ROOT",
                "GATA_MODULEC_OUTPUT_ROOT",
                "OUTPUT_ROOT",
            ])

        if key in ("output_root", "out_root", "output_dir", "out_dir"):
            return _r7q_env_first([
                "MODULEC_OUTPUT_ROOT",
                "GATA_MODULEC_OUTPUT_ROOT",
                "OUTPUT_ROOT",
        ...
        if "data" in key or "datos" in key or "input" in key:
            return _r7q_env_first([
                "MODULEC_DATA_ROOT",
        ...
        if "runtime" in key:
            return _r7q_env_first([
                "MODULEC_RUNTIME_ROOT",
                "RUNTIME_ROOT",
            ])

        if key in ("run_root", "runroot") or ("run" in key and "root" in key):
            return _r7q_env_first([
                "MODULEC_RUN_ROOT",
                "RUN_ROOT",
                "MODULEC_RUNTIME_ROOT",
        ...
        if "repo" in key and "root" in key:
            return _r7q_env_first([
                "MODULEC_REPO_ROOT",
                "GATA_REPO_ROOT",
                "REPO_ROOT",
    ...
    def _r7q_option_present(raw_args, option_strings):
        raw_args = list(raw_args or [])
        for opt in option_strings or []:
            if opt in raw_args:
                return True
            prefix = str(opt) + "="
            for arg in raw_args:
    ...
    def _r7q_write_trace(name, text):
        runtime_root = _r7q_env_first(["MODULEC_RUNTIME_ROOT", "RUNTIME_ROOT"])
        if not runtime_root:
            return
        try:
            log_dir = _R7QPath(runtime_root) / "logs"
    ...
    _r7q_original_parse_args = _r7q_argparse.ArgumentParser.parse_args
    _r7q_original_error = _r7q_argparse.ArgumentParser.error

    def _r7q_patched_error(self, message):
        try:
            _r7q_write_trace(
                "r7q_argparse_error.txt",
                "ARGPARSE_ERROR=" + str(message) + "\nUSAGE=" + str(self.format_usage())
            )
        except Exception:
            pass
        return _r7q_original_error(self, message)

    def _r7q_patched_parse_args(self, args=None, namespace=None):
        raw = list(_r7q_sys.argv[1:] if args is None else args)
        injected = []

        try:
            actions = list(getattr(self, "_actions", []) or [])
        except Exception:
            actions = []

        for action in actions:
            dest = getattr(action, "dest", "")
            if dest in ("help", _r7q_argparse.SUPPRESS):
                continue

            value = _r7q_value_for_dest(dest)
            if not value:
                continue
            ...
            if option_strings:
                if _r7q_option_present(raw, option_strings):
                    continue
                ...
                if is_required or is_runtime_contract:
                    if nargs == 0:
                        continue
                    preferred = None
                    ...
                    if preferred is None:
                        preferred = option_strings[0]

                    raw.extend([preferred, value])
                    injected.append((dest, preferred, value))
                ...
                is_known_positional = (
                    ...
                    "input" in key or
                    "runtime" in key or
                    key in ("run_root", "runroot")
                )

                if is_known_positional and value not in raw:
                    raw.append(value)
                    injected.append((dest, "<positional>", value))

        if injected:
            _r7q_write_trace("r7q_argparse_injections.txt", "INJECTED=" + repr(injected))
            _r7q_write_trace("r7q_argparse_injections.txt", "ARGV_AFTER=" + repr(raw))

        ns = _r7q_original_parse_args(self, raw, namespace)

        try:
            if not hasattr(ns, "output_root"):
                out = _r7q_env_first(["MODULEC_OUTPUT_ROOT", "GATA_MODULEC_OUTPUT_ROOT", "OUTPUT_ROOT"])
                if out:
                    setattr(ns, "output_root", out)

            if not hasattr(ns, "data_root"):
                data = _r7q_env_first(["MODULEC_DATA_ROOT", "GATA_MODULEC_DATA_ROOT", "DATA_ROOT"])
                if data:
                    setattr(ns, "data_root", data)

            if not hasattr(ns, "runtime_root"):
                runtime = _r7q_env_first(["MODULEC_RUNTIME_ROOT", "RUNTIME_ROOT"])
                if runtime:
                    setattr(ns, "runtime_root", runtime)

            if not hasattr(ns, "run_root"):
                run_root = _r7q_env_first(["MODULEC_RUN_ROOT", "RUN_ROOT"])
                if run_root:
                    setattr(ns, "run_root", run_root)
        except Exception:
            pass
    ...
    try:
        ...
        _r7q_write_trace("r7q_runtime_contract_trace.txt", "ARGV_INITIAL=" + repr(_r7q_sys.argv))
        _r7q_write_trace("r7q_runtime_contract_trace.txt", "MODULEC_OUTPUT_ROOT=" + str(_r7q_os.environ.get("MODULEC_OUTPUT_ROOT", "")))
        _r7q_write_trace("r7q_runtime_contract_trace.txt", "MODULEC_DATA_ROOT=" + str(_r7q_os.environ.get("MODULEC_DATA_ROOT", "")))
        _r7q_write_trace("r7q_runtime_contract_trace.txt", "MODULEC_RUNTIME_ROOT=" + str(_r7q_os.environ.get("MODULEC_RUNTIME_ROOT", "")))
        _r7q_write_trace("r7q_runtime_contract_trace.txt", "MODULEC_RUN_ROOT=" + str(_r7q_os.environ.get("MODULEC_RUN_ROOT", "")))
    except Exception:
        pass
...
OC03C_BASE_SMOKE_EXPECTED_UNIQUE_YEARS = 10
OC03C_BASE_SMOKE_EXPECTED_UNIQUE_UNITS = 26
...
def run_base_smoke_contract_for_oc03c(output_root: Path) -> Dict[str, object]:
    ...
    add(
        "daily_rows",
        daily_rows_metric if daily_rows_metric is not None else "MISSING",
        daily_rows_metric == OC03C_BASE_SMOKE_EXPECTED_DAILY_ROWS,
        OC03C_BASE_SMOKE_EXPECTED_DAILY_ROWS,
        "Decoder daily spatial audit rows.",
    )
    add(
        "unique_dates",
        unique_dates_metric if unique_dates_metric is not None else "MISSING",
        unique_dates_metric == OC03C_BASE_SMOKE_EXPECTED_UNIQUE_DATES,
        OC03C_BASE_SMOKE_EXPECTED_UNIQUE_DATES,
        "Decoder daily spatial audit unique_dates.",
    )
    add(
        "unique_years",
        unique_years_metric if unique_years_metric is not None else "MISSING",
        unique_years_metric == OC03C_BASE_SMOKE_EXPECTED_UNIQUE_YEARS,
        OC03C_BASE_SMOKE_EXPECTED_UNIQUE_YEARS,
        "Decoder daily spatial audit unique_years.",
    )
    add(
        "unique_units",
        unique_units_metric if unique_units_metric is not None else "MISSING",
        unique_units_metric == OC03C_BASE_SMOKE_EXPECTED_UNIQUE_UNITS,
        OC03C_BASE_SMOKE_EXPECTED_UNIQUE_UNITS,
        "Decoder daily spatial audit unique_units.",
    )
    add(
        "homogeneous_years",
        homogeneous_years_metric if homogeneous_years_metric is not None else "MISSING",
        homogeneous_years_metric == OC03C_BASE_SMOKE_EXPECTED_HOMOGENEOUS_YEARS,
        OC03C_BASE_SMOKE_EXPECTED_HOMOGENEOUS_YEARS,
        "Decoder daily spatial audit homogeneous_years.",
    )

    daily_years = _extract_years(daily_rows)
    ...
    daily_table_ok = daily_table_row_count == OC03C_BASE_SMOKE_EXPECTED_DAILY_ROWS and daily_rows_metric == daily_table_row_count
    add(
        "daily_table_rows",
        daily_table_row_count,
        daily_table_ok,
        OC03C_BASE_SMOKE_EXPECTED_DAILY_ROWS,
        f"decoder_daily_rows={daily_rows_metric if daily_rows_metric is not None else 'MISSING'}",
    )
    ...
    single_year_fallback_detected = bool((unique_years_metric or 0) <= 1 or len(daily_years) <= 1)
    add(
    ...
    return {
        ...
        "base_smoke_contract_for_oc03c_daily_rows": daily_rows_metric or 0,
        "base_smoke_contract_for_oc03c_unique_dates": unique_dates_metric or 0,
        "base_smoke_contract_for_oc03c_unique_years": unique_years_metric or 0,
        "base_smoke_contract_for_oc03c_unique_units": unique_units_metric or 0,
        "base_smoke_contract_for_oc03c_homogeneous_years": homogeneous_years_metric or 0,
