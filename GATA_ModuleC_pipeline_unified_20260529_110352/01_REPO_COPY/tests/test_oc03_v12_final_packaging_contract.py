...
def test_pipeline_rebuilds_and_verifies_final_package_after_step9_refresh():
    ...
    assert text.count("build_manifest_and_zip(outputs, deliver_dir, report)") >= 2
    assert "QA gate decision (post-step9)" in text
    assert 'report.log("Final deliverables package verified against manifest, SHA checkpoints, and ZIP members.")' not in text
